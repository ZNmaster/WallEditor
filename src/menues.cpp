...
void wxGUI2Frame::CreateConnectMenu()
{
    wxMenu* connectMenu = new wxMenu(_T(""));

    //file menu
    //connectMenu->Append(idMenuFile_New, _("&New\tCtrl-N"), _("New project"));
    //connectMenu->Append(idMenuFile_Open, _("&Open\tCtrl-O"), _("Open project"));
    //connectMenu->Append(idMenuFile_Save, _("&Save\tCtrl-S"), _("Save project"));
    connectMenu->AppendSeparator();

    wxMenu* conn = new wxMenu;
    //import sub menu
    conn->Append(idMenuConnect_Server, _("&Server"), _("Connect to a server"));
    conn->Append(idMenuConnect_Unknown, _("&Unknown Server"), _("Connect to a server with unknown protocol"));

    connectMenu->AppendSubMenu(conn, wxT("&Connect"));

    connectMenu->AppendSeparator();
    //connectMenu->Append(idMenuQuit, _("&Quit\tAlt-F4"), _("Quit the application"));
    mbar->Append(connectMenu, _("&Connect"));
    return;
}
